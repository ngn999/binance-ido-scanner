...
const TARGET_SPENDER_ADDRESS = ethers.getAddress(
	"0xb300000b72deaeb607a12d5f54773d1c19c7028d",
); // Use getAddress for checksum validation
const APPROVE_FUNC_SIGNATURE = "approve(address,uint256)";
const APPROVE_FUNC_SELECTOR = ethers
	.id(APPROVE_FUNC_SIGNATURE)
	.substring(0, 10); // Calculates "0x095ea7b3"
...
async function scanRecentApprovals() {
	...
	try {
		...
		console.log(
			`Scanning blocks from ${startBlock} to ${latestBlockNumber}...`,
		);
		...
		for (
			...
			const endBatchBlock = Math.min(
				currentBlock + BATCH_SIZE - 1,
				latestBlockNumber,
			);
				...
				for (const tx of block.prefetchedTransactions) {
					...
					if (tx.to && tx.data && tx.data.startsWith(APPROVE_FUNC_SELECTOR)) {
						try {
							...
							const decodedData = approveInterface.parseTransaction({
								data: tx.data,
							});
							...
							console.warn(
								`  Could not decode transaction data for ${tx.hash}: ${decodeError}`,
							);
						}
					}
				}
...
if (!BSC_RPC_URL || BSC_RPC_URL === "YOUR_BSC_RPC_URL") {
	...
	console.error(
		"Please replace 'YOUR_BSC_RPC_URL' in the script or set the BSC_RPC_URL environment variable.",
	);
} else {
